(function(){
  ...
  // no longer hardcoded here. Was previously a third independent copy of
  // the same data duplicated in IncomeCalculatorSimple/app.js and
  // InvestmentGrowthCalculator/app.js; all three now read from the same
  // server-side source of truth. Everything below still computes fully
  // locally/synchronously against the fetched data.
  ...
  // ---------- init ----------
  // Nothing below runs until reference data arrives — computeTaxesForPerson
  // would throw against still-undefined FEDERAL_BRACKETS/STATE_TAX.
